...
public class AdminPointService {
	...
	public Map<String, Object> getPointRefundStandard(int currentPageRefund){
		...
		// 전체 행의 개수
		double rowsCount = adminPointMapper.getPointRefundStandardCount();
	...
	public Map<String, Object> getPointSaveStandard(int currentPageSave){
		...
		// 전체 행의 개수
		double rowsCount = adminPointMapper.getPointSaveStandardCount();
	...
	/* 3-2 포인트 타입 기준 조회 */
	public Map<String, Object> getPointTypeStandard(int currentPageType){
		
		// 한 번에 최대로 보여줄 행의 개수
		int rowPerPage = 20;
		
		// 현재 페이지의 첫 번째 행의 인덱스
		int currentFirstIndex = (currentPageType-1) * rowPerPage;		
		
		// 전체 행의 개수
		double rowsCount = adminPointMapper.getPointTypeStandardCount();
		...
		// 동적 페이지 구성(7페이지 부터)
		if(currentPageType >= 7 && lastPageNum > 10) {
			startPageNum = currentPageType - 5;
			endPageNum = currentPageType + 4;
			if (endPageNum >= lastPageNum){
				startPageNum = lastPageNum - 9;
				endPageNum = lastPageNum;				
			}
		...
		paramMap.put("lastPageNumType", lastPageNum);
		paramMap.put("startPageNumType", startPageNum);
		paramMap.put("endPageNumType", endPageNum);
	...
	public Map<String, Object> getPointExpireStandard(int currentPageExpire){
		...
		// 전체 행의 개수
		double rowsCount = adminPointMapper.getPointExpireStandardCount();
